...
const Checkout = () => {
  ...
  const validarOrden = (orden) => {
    ...
    if (orden.nombre.length < 2) {
      console.log(orden.nombre.length)
      console.log(nombre)
    ...
    if (orden.apellido.length < 2) {
      console.log(orden.apellido.length)
      toast.error("Apellido no válido, ingrese un apellido con más de dos caracteres");
    } else {
      apellido = true;
    }
  
    if (orden.ciudad.length < 2) {
      console.log(orden.ciudad.length)
    ...
    if (orden.email.indexOf("@") === -1) {
      toast.error("Email no válido, ingrese un email del formato mi@correo.com");
    } else {
      email = true;
    }
