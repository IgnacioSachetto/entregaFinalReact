...
Para agregar items al carrito el usuario puede agregarlo ingresando al detalle del producto y seleccionando la cantidad, el producto y la cantidad se verán reflejados en el carrito. Cuando el usuario ingrese al carrito podrá agregar más cantidades del producto si lo considera.Para disminuir la cantidad inicial el usuario deberá eliminar el producto del carrito y volver a ingresarlo con la cantidad deseada. Otra alternativa podría haber sido actualizar dinamicamente la cantidad cada vez que el usuario aprete el boton de (+) o (-) decidí optar por la primer opción aunque se podría modificar en caso que el tutor/profesor no lo considere apropiado.
